      ...
      try {
        ...
        if (body.includes(suffix)) {
          chrome.runtime.sendMessage({
            ...
            desc: "This password was found in a known data breach!",
...
window.addEventListener("message", (e) => {
  ...
  if (e.data.type === "GUARDRAIL_MEDIA_INTERNAL") {
    chrome.runtime.sendMessage({
      ...
      details: {
        kind: "media-access",
        constraints: sanitizeForLog(e.data.constraints),
      },
    });
  }
  ...
  if (e.data.type === "GUARDRAIL_JS_EVENT") {
    chrome.runtime.sendMessage({
      ...
      desc:
        e.data.desc ||
        "The page registered suspicious background JavaScript behavior.",
      actions: e.data.domain
        ? [
            {
              type: "block-domain",
