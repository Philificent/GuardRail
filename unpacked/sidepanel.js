...
function render() {
  chrome.storage.local.get(
    ...
    (data) => {
      ...
      spoofProfileCopy.textContent = formatSpoofProfile(
        data.spoofEnabled,
        data.spoofProfile,
      );
      ...
      const filtered = data.logs.filter((l) => {
        ...
        return (
          l.site.toLowerCase().includes(search) ||
            ...
            actions.push(
              createSafeElement(
                ...
                blocked
                  ...
                  : action.label || "Add to Blocklist",
              ),
            );
          }
        });

        const card = createSafeElement(
          "div",
          { className: `card ${l.severity}` },
          null,
          [
            ...
            ...(actions.length
              ...
              : []),
            ...(openDetailId === l.id && l.details
              ? [createInlineDetails(l.details)]
              : []),
          ],
        );
      ...
      document.getElementById("blocklistPreview").textContent = data.blocklist
        .length
...
function formatSpoofProfile(enabled, profile) {
  ...
  const refererValue =
    activeProfile.referer === "removed"
      ? "Referer removed"
      : `Referer ${activeProfile.referer}`;
...
jsKillswitchToggle.addEventListener("change", () => {
  ...
  chrome.runtime.sendMessage({
    type: "TOGGLE_JS_KILLSWITCH",
    enabled: jsKillswitchToggle.checked,
  });
});
...
document
  .getElementById("addBlocklistBtn")
  .addEventListener("click", async () => {
    ...
    const actionType = currentBlocklist.includes(normalized)
...
document
  .getElementById("bulkAddBlocklistBtn")
  .addEventListener("click", async () => {
    const domains = parseDomains(bulkBlocklistInput.value);
    const response = await chrome.runtime.sendMessage({
      type: "ADD_BLOCKLIST_BULK",
      domains,
    });
    if (!response?.ok) {
...
logDiv.addEventListener("click", async (event) => {
  ...
  if (
    button.dataset.action === "remove-managed-domain" &&
    button.dataset.domain
  ) {
    const response = await chrome.runtime.sendMessage({
      type: "REMOVE_BLOCKLIST_DOMAIN",
      domain: button.dataset.domain,
    });
    if (!response?.ok) {
