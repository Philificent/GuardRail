...
const functionMatch = sidepanelJs.match(
  /function normalizeDomain\(value\) \{[\s\S]*?\n\}/,
);
if (!functionMatch) {
...
function runTests() {
  const tests = [
    ...
    },
    {
      ...
      desc: "Case sensitivity and subdomains",
    },
    ...
    { input: "   ", expected: null, desc: "Whitespace string" },
    {
      input: "https://[",
      expected: null,
      desc: "Malformed URL (triggers catch block)",
    },
  ];
  ...
  for (const t of tests) {
    ...
    if (result === t.expected) {
      console.log(
        `✅ PASS: [${t.desc}] input="${t.input}" -> result="${result}"`,
      );
      passed++;
    } else {
      console.error(
        `❌ FAIL: [${t.desc}] input="${t.input}" -> Expected: "${t.expected}", Got: "${result}"`,
      );
    }
  }
